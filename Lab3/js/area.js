...
// The code is taken from the example on the site below.
...
function area(data) {
    ...
    // Goes in the order year, month, day, hour etc.
    ...
    var area = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                // Get the time from the data set
                // using the "x" scale.
                return x(format(d.time));;
            })
            .y0(height)
            .y1(function (d) {
                // Get the magnitude from the data set.
                // using the "y" scale.
    ...
    // The x-axis uses the extent of the time and
    // the y-axis uses the exent of the magnitude.
    ...
    function brush() {
        ...
        // Call the function "filterTime", which filters so that
        // the values from the small time chart will be drawn in the
        // map. "brush.extent()" represents the start and end time.
